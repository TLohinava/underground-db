...
CREATE TABLE IF NOT EXISTS `undergrounds`.`carriages` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE INDEX `carriage_number_UNIQUE` (`carriage_number` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `undergrounds`.`ticket_boxes` (
  ...
  PRIMARY KEY (`id`),
  INDEX `fk_ticket_boxes_stations1_idx` (`stations_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `undergrounds`.`train_drivers` (
  ...
  INDEX `fk_trains_has_drivers_trains1_idx` (`train_id` ASC) VISIBLE,
  CONSTRAINT `fk_trains_has_drivers_trains1`
    FOREIGN KEY (`train_id`)
    REFERENCES `undergrounds`.`trains` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
